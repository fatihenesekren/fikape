...
export function ExpertNoteForm({ headline }: { headline: string | null }) {
  ...
  // Kullanıcı fark etti: 2 harf yazınca hiçbir şey olmuyormuş GİBİ
  // görünüyordu — 0 sonuç dönünce dropdown hiç render edilmiyordu (ne
  // "aranıyor" ne "sonuç yok" mesajı vardı, tamamen sessizdi).
  const [searching, setSearching] = useState(false);

  useEffect(() => {
    const q = query.trim();
    if (selected || q.length < 2) {
      const t = setTimeout(() => { setSearching(false); setResults([]); setDropdownOpen(false); }, 0);
    ...
    return () => { clearTimeout(t); clearTimeout(searchingTimer); };
  }, [query, selected]);
          ...
          {selected ? (
            ...
            <>
              <div className="relative">
                ...
                />
                {searching && (
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400">Aranıyor…</span>
                )}
              </div>
              ...
              {dropdownOpen && !searching && results.length === 0 && query.trim().length >= 2 && (
                <div className="absolute z-10 mt-1 w-full bg-white border border-gray-100 rounded-xl shadow-lg px-3 py-2.5 text-sm text-gray-400">
