...
const VALID_TARGET_TYPES = ["SPEC", "PHOTO", "REVIEW", "QNA", "EXPERT_NOTE", "OTHER"] as const;
...
export async function POST(req: Request) {
  ...
  const { productId, targetType, field, photoId, reviewId, questionId, expertNoteId, note } = await req.json();
  ...
  // Usta notu bildirimi ürüne değil MODEL seviyesindeki nota bağlanır —
  // schema'daki CHECK constraint gereği productId NULL, expertNoteId dolu
  // olmalı (bkz. prisma/schema.prisma ContentReport yorumu).
  const productIdNum = Number(productId);
  if (type !== "EXPERT_NOTE" && !Number.isInteger(productIdNum)) {
    return NextResponse.json({ error: "Geçersiz araç." }, { status: 400 });
  }

  await prisma.contentReport.create({
    data: {
      productId: type === "EXPERT_NOTE" ? null : productIdNum,
